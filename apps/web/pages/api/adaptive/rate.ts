...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!itemId || typeof rating !== "number")
      return errorResponse(res, 400, "INVALID_PAYLOAD");
    ...
    if (rating <= 0) nextInterval = 10;
      ...
      nextInterval = Math.max(30, parseInt(String(lastInterval * 1.4)) || 30);
    else
      nextInterval = Math.max(60, parseInt(String(lastInterval * 2.2)) || 60);
