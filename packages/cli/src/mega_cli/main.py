...
def ingest():
    ...
    typer.echo(
        json.dumps(
            mods, ensure_ascii=False, indent=2 if CONFIG.cli.json_pretty else None
        )
    )
...
@app.command()
def draft_case(
    topic: str = typer.Argument(..., help="Tópico para caso clínico (placeholder)"),
):
    typer.echo(
        json.dumps(
            {"topic": topic, "status": "placeholder"},
            ensure_ascii=False,
